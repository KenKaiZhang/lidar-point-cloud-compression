#include "utils.h"
void load_pcloud_ply(std::string &file_name, std::vector<point_cloud> &pcloud_data)
{
  ...
  {
    if (p.x * p.x + p.y * p.y + p.z * p.z < 2.0f)
      continue;
    pcloud_data.push_back(point_cloud(p.x, p.y, p.z, 0.0f));
  }
}
...
void export_pcloud(std::string file_name, std::vector<point_cloud> &pcloud_data)
{
  ...
  for (const auto &point : pcloud_data)
  {
    float data[4] = {point.x, point.y, point.z, point.r};
    fwrite(data, sizeof(float), 4, stream);
  }
...
float neighbor_search(float radius, cv::Mat &img, int row, int col)
{
  ...
  for (int i = std::max(0, row - 2); i < std::min(img.rows, row + 2); i++)
  {
    for (int j = std::max(0, col - 2); j < std::min(img.cols, col + 2); j++)
    {
                        ...
                        float pitch_precision, float yaw_precision)
{
  ...
  for (auto point : pcloud)
  {
    ...
    if (std::isnan(restored_radius) || (restored_radius > 300.0f) || restored_radius < 1.0f)
    {
                    ...
                    std::vector<point_cloud> &restored_pcloud)
{
  ...
  for (int row = 0; row < img.rows; row++)
  {
    for (int col = 0; col < img.cols; col++)
    {
      ...
      auto p = point_cloud(x, y, z, radius);

      restored_pcloud.push_back(p);
    }
  }
}
